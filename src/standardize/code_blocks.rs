...
fn detect_lang_from_class(class: &str) -> Option<String> {
    ...
    if tokens.contains(&"hl") && tokens.contains(&"block") {
        for t in &tokens {
...
fn build_canonical_pre(text: &str, lang: Option<&str>) -> NodeRef {
    ...
    let attrs = lang.map_or_else(Vec::new, |l| {
        let lang_class = format!("language-{l}");
...
fn looks_like_inline_lineno(node: &NodeRef) -> bool {
    ...
    let mut prev_sib = node.previous_sibling();
    while let Some(p) = prev_sib {
    ...
    let parent_class = node
        ...
        .unwrap_or_default()
        .to_ascii_lowercase();
    let has_parent_hint = parent_class
        .split_whitespace()
        .any(|t| t == "flex-row" || t == "ec-line" || t == "line" || t == "cl")
        && node.next_sibling().is_some();
    has_style_hint || has_class_hint || has_parent_hint
}

/// Recursive helper for [`extract_text_with_lines`]. `last_was_line_term`
/// tracks whether the most recent emitted child element terminated its own
/// line (a line container produced "...\n", or a previous `<br>` already
/// wrote a newline). Used to decide whether a following `<br>` should add
/// another newline.
fn extract_text_with_lines_visit(n: &NodeRef, out: &mut String, last_was_line_term: &mut bool) {
    for c in n.children() {
        if let Some(el) = c.as_element() {
            ...
            // tooltips that some highlighters bake into the DOM.
            {
                ...
                if attrs_borrow.get("aria-hidden").is_some_and(|v| v == "true")
            ...
            let aria_hidden = el
                ...
                .unwrap_or_default();
            if aria_hidden == "true"
                && (text_content(&c).trim().chars().all(|x| x.is_ascii_digit()))
            {
                continue;
            }
            ...
            extract_text_with_lines_visit(&c, out, &mut child_lwlt);
...
fn extract_text_with_lines(node: &NodeRef) -> String {
    ...
    let mut state = false;
    extract_text_with_lines_visit(node, &mut out, &mut state);
    out
}
...
/// Pass A: `<code style="white-space:pre">` not inside a `<pre>` → wrap in
/// `<pre>`. Track-D's `elements::normalize_all` (`src/elements/code.rs`)
/// already handles Chroma/Pygments/Shiki/CodeMirror normalisation
/// downstream of this pass.
fn pass_a_wrap_bare_code(root: &NodeRef) {
...
fn pass_a2_hexo_figures(root: &NodeRef) {
    let hexo_figs: Vec<NodeRef> = root.descendants().filter(looks_like_hexo_figure).collect();
...
/// Pass A3: preceding-sibling clean-up. For every `<pre>` in the tree, walk
/// up a few parents and detach short text labels / language-name spans that
/// appear *before* the pre as siblings of the pre or of an ancestor
/// wrapper. This removes the "java"-style label headers from
/// react-syntax-highlighter and similar wrappers even after legacy flatten
/// has dissolved their original container.
fn pass_a3_strip_preceding_labels(root: &NodeRef) {
...
/// True if `cand` — a `<pre>` or chroma/codemirror `<div>` — has any nested
/// decoration the markdown layer would otherwise render verbatim, and so
/// needs normalizing to a canonical `<pre><code>` shape.
fn needs_normalize(cand: &NodeRef, is_pre: bool) -> bool {
    if !is_pre {
        return true;
    }
    ...
    let has_br_in_code = cand.descendants().any(|d| is_any_tag(&d, &["br"]));
    ...
    let has_div_children = cand.children().any(|c| is_any_tag(&c, &["div"]))
...
fn pass_b_canonicalize(root: &NodeRef) {
    let candidates: Vec<NodeRef> = root
        ...
        .filter(|d| {
            if let Some(name) = tag_name(d) {
                ...
                if name == "div" && (looks_like_chroma_or_highlight(d) || looks_like_codemirror(d))
                {
                    return true;
                }
            }
    ...
    for cand in candidates {
        ...
        if !needs_normalize(&cand, name == "pre") {
            continue;
        }
...
/// Pass B2: detach copy buttons / language label spans that sit adjacent to
/// a `<pre>` (typical of react-syntax-highlighter and similar wrappers
/// where the chrome is a sibling of the code).
fn pass_b2_strip_buttons_and_labels(root: &NodeRef) {
    ...
    for s in lang_label_spans {
        ...
        if parent.descendants().any(|d| is_any_tag(&d, &["pre"])) && !is_any_tag(&parent, &["pre"])
        {
            ...
            let mut prev_sib = s.previous_sibling();
            let mut leftmost = true;
            while let Some(p) = prev_sib.take() {
                if let Some(t) = p.as_text() {
                    if t.borrow().chars().all(char::is_whitespace) {
                        prev_sib = p.previous_sibling();
                        continue;
...
fn pass_c_strip_chrome(root: &NodeRef) {
    for node in descendants_post_order(root) {
        if !is_any_tag(&node, &["div", "button", "span"]) {
            continue;
        }
        let Some(class) = get_attr(&node, "class") else {
            continue;
        };
        let lc = class.to_ascii_lowercase();
...
/// Pass C1: for any `<pre><code class="block ...">` lean-verso-style shape,
/// force normalize so leading/trailing blank lines and syntax-highlight
/// spans are collapsed.
fn pass_c1_lean_verso_blocks(root: &NodeRef) {
    let block_codes: Vec<NodeRef> = root
        ...
        .filter(|d| {
            ...
            class.split_whitespace().any(|t| t == "block")
                && d.parent().is_some_and(|p| is_any_tag(&p, &["pre"]))
        })
        .collect();
    for code in block_codes {
        let Some(pre) = code.parent() else {
            continue;
        };
...
fn pass_c2_rewrite_lang_classes(root: &NodeRef) {
    for node in descendants_post_order(root) {
        if !is_any_tag(&node, &["code"]) {
            continue;
        }
...
fn pass_d_ensure_code_child(root: &NodeRef) {
    for node in descendants_post_order(root) {
        ...
        let code_attrs = lang.map_or_else(Vec::new, |l| {
            let lang_class = format!("language-{l}");
            crate::dom::walk::build_attrs(&[("class", lang_class.as_str())])
        });
...
impl DomPass for CodeBlocks {
    ...
    fn run(&self, root: &NodeRef, _ctx: &DomCtx) {
        pass_a_wrap_bare_code(root);
        pass_a2_hexo_figures(root);
        pass_a3_strip_preceding_labels(root);
        pass_b_canonicalize(root);
        pass_b2_strip_buttons_and_labels(root);
        pass_c_strip_chrome(root);
        pass_c1_lean_verso_blocks(root);
        pass_c2_rewrite_lang_classes(root);
        pass_d_ensure_code_child(root);
    }
}
