//! The node tree: `NodeRef`, `Node`, and `NodeData`.
//!
//! Vendored from `kuchikiki` (see the attribution notice in
//! `crate::dom::engine`), unchanged apart from `use` paths.

use html5ever_engine::tree_builder::QuirksMode;
// `expanded_name!` expands to unqualified `ns!`/`local_name!` calls, so
// both macros must also be in scope wherever `expanded_name!` is invoked.
use html5ever_engine::{QualName, expanded_name, local_name, ns};
use std::cell::{Cell, RefCell};
use std::fmt;
use std::ops::Deref;
use std::rc::{Rc, Weak};

use crate::dom::engine::attributes::{Attribute, Attributes, ExpandedName};
use crate::dom::engine::cell_extras::*;
use crate::dom::engine::iter::NodeIterator;
...
///
/// Each node holds a strong reference to its first child and next sibling
/// (if any), but only a weak reference to its last child, previous
/// sibling, and parent. This is to avoid strong reference cycles, which
/// would cause memory leaks.
///
/// As a result, a single `NodeRef` is sufficient to keep alive a node and
/// nodes that are after it in tree order (its descendants, its following
/// siblings, and their descendants) but not other nodes in a tree.
///
/// To avoid destroying nodes prematurely, programs typically hold a strong
/// reference to the root of a document until they're done with that
/// document.
#[derive(Clone, Debug)]
...
/// Prevent implicit recursion when dropping nodes to avoid overflowing the
/// stack.
///
/// The implicit drop is correct, but recursive. In the worst case (where
/// no node has both a next sibling and a child), a tree of a few tens of
/// thousands of nodes could cause a stack overflow.
///
/// This `Drop` implementation makes sure the recursion does not happen.
/// Instead, it has an explicit `Vec<Rc<Node>>` stack to traverse the
/// subtree, but only following `Rc<Node>` references that are "unique":
/// that have a strong reference count of 1. Those are the nodes that
/// would have been dropped recursively.
///
/// The stack holds ancestors of the current node rather than preceding
/// siblings, on the assumption that large document trees are typically
/// wider than deep.
impl Drop for Node {
    fn drop(&mut self) {
        // `.take_if_unique_strong()` temporarily leaves the tree in an
        // inconsistent state, as the corresponding `Weak` reference in the
        // other direction is not removed. It is important that all
        // `Some(_)` strong references it returns are dropped by the end
        // of this `drop` call, and that no user code is invoked
        // in-between.

        // Sharing `stack` between these two calls is not necessary, but
        // it allows re-using memory allocations.
        ...
        fn non_recursive_drop_unique_rc(mut rc: Rc<Node>, stack: &mut Vec<Rc<Node>>) {
            loop {
                ...
                if let Some(sibling) = rc.next_sibling.take_if_unique_strong() {
                    // The previous value of `rc: Rc<Node>` is dropped
                    // here. Since it was unique, the corresponding `Node`
                    // is dropped as well. `<Node as Drop>::drop` does not
                    // call `drop_rc` as both the first child and next
                    // sibling were already taken. Weak reference counts
                    // decremented here for `Cell`s that are `Some`:
                    // * `rc.parent`: still has a strong reference in
                    //   `stack` or elsewhere
                    // * `rc.last_child`: this is the last weak ref.
                    //   Deallocated now.
                    // * `rc.previous_sibling`: this is the last weak ref.
                    //   Deallocated now.
                    rc = sibling;
                    continue;
                }
...
impl Node {
    ...
    /// If this node is an element, return a reference to element-specific
    /// data.
    #[inline]
    ...
    /// If this node is a doctype, return a reference to doctype-specific
    /// data.
    #[inline]
    ...
    /// Return a reference to the parent node, unless this node is the
    /// root of the tree.
    #[inline]
    ...
    /// Return a reference to the first child of this node, unless it has
    /// no child.
    #[inline]
    ...
    /// Return a reference to the last child of this node, unless it has
    /// no child.
    #[inline]
    ...
    /// Return a reference to the previous sibling of this node, unless it
    /// is a first child.
    #[inline]
    ...
    /// Return a reference to the next sibling of this node, unless it is
    /// a last child.
    #[inline]
    ...
    /// affected.
    ///
    /// To remove a node and its descendants, detach it and drop any
