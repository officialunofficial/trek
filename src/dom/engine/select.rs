//! CSS selector compilation and matching, backed by the `selectors` and
//! `cssparser` crates.
//!
//! Vendored from `kuchikiki` (see the attribution notice in
//! `crate::dom::engine`), unchanged apart from `use` paths.

use crate::dom::engine::attributes::ExpandedName;
use crate::dom::engine::iter::{NodeIterator, Select};
use crate::dom::engine::node_data_ref::NodeDataRef;
use crate::dom::engine::tree::{ElementData, Node, NodeData, NodeRef};
use cssparser::{self, CowRcStr, ParseError, SourceLocation, ToCss};
use html5ever_engine::{LocalName, Namespace, local_name, ns};
...
impl SelectorImpl for EngineSelectors {
...
impl<'i> Parser<'i> for EngineParser {
    type Impl = EngineSelectors;
...
impl selectors::Element for NodeDataRef<ElementData> {
    ...
    fn match_pseudo_element(
        ...
        _context: &mut matching::MatchingContext<EngineSelectors>,
    ) -> bool {
        match *pseudo {}
    }
...
pub struct Selector(GenericSelector<EngineSelectors>);

/// The specificity of a selector.
///
/// Opaque, but ordered.
///
/// Determines precedence in the cascading algorithm. When equal, a rule
/// later in source order takes precedence.
#[derive(Copy, Clone, Hash, Eq, PartialEq, Ord, PartialOrd)]
pub struct Specificity(u32);

impl Selectors {
    /// Compile a list of selectors. This may fail on syntax errors or
    ...
    /// Filter an element iterator, yielding those matching this list of
    /// selectors.
    #[inline]
