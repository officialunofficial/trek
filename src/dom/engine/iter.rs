...
use crate::dom::engine::node_data_ref::NodeDataRef;
use crate::dom::engine::select::Selectors;
use crate::dom::engine::tree::{ElementData, NodeRef};
...
impl NodeRef {
    ...
    /// Return an iterator of references to this node and the siblings
    /// before it.
    #[inline]
    ...
    /// Return an iterator of references to this node's siblings before
    /// it.
    #[inline]
    ...
    /// Return an iterator of the start and end edges of this node and
    ...
    /// Return an iterator of the start and end edges of this node's
    /// descendants, in tree order.
    #[inline]
    ...
    /// Return an iterator of the inclusive descendants element that
    /// match the given selector list.
    #[inline]
    pub fn select(&self, selectors: &str) -> Result<Select<Elements<Descendants>>, ()> {
        self.inclusive_descendants().select(selectors)
    }

    /// Return the first inclusive descendants element that match the
    /// given selector list.
    #[inline]
...
pub enum NodeEdge<T> {
    ...
    /// `Traverse::next` before the node's descendants. In HTML or XML,
    /// this corresponds to an opening tag like `<div>`
    Start(T),

    /// Indicates that end of a node that has children. Yielded by
    /// `Traverse::next` after the node's descendants. In HTML or XML,
    /// this corresponds to a closing tag like `</div>`
    End(T),
}

/// An iterator of the start and end edges of the nodes in a given
/// subtree.
#[derive(Debug, Clone)]
...
/// An element iterator adaptor that yields elements matching given
/// selectors.
...
pub trait NodeIterator: Sized + Iterator<Item = NodeRef> {
    ...
    /// Filter this node iterator to elements matching the given
    /// selectors.
    #[inline]
