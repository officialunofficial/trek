//! `NodeDataRef`: a strong reference to a node that dereferences to one of
//! its data components.
//!
//! Vendored from `kuchikiki` (see the attribution notice in
//! `crate::dom::engine`), unchanged apart from `use` paths.

use crate::dom::engine::tree::{Doctype, DocumentData, ElementData, Node, NodeRef};
...
impl NodeRef {
    ...
    /// If this node is a text node, return a strong reference to its
    /// contents.
    #[inline]
    ...
    /// If this node is a comment, return a strong reference to its
    /// contents.
    #[inline]
...
/// Holds a strong reference to a node, but dereferences to some component
/// inside of it.
#[derive(Eq)]
...
impl<T> NodeDataRef<T> {
    ...
    /// Create a `NodeDataRef` for a component that may or may not be in a
    /// given node.
    #[inline]
