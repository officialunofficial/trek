//! HTML serialization for `NodeRef`.
//!
//! Vendored from `kuchikiki` (see the attribution notice in
//! `crate::dom::engine`), unchanged apart from `use` paths.

use html5ever_engine::QualName;
use html5ever_engine::serialize::TraversalScope::*;
use html5ever_engine::serialize::{
...
use crate::dom::engine::tree::{NodeData, NodeRef};
        ...
        match (traversal_scope, self.data()) {
            (ref scope, NodeData::Element(element)) => {
                if *scope == IncludeNode {
                    ...
                    // Unfortunately we need to allocate something to hold
                    // these &'a QualName
                    let attrs = attrs
                        .map
                        .iter()
...
impl NodeRef {
    /// Serialize this node and its descendants in HTML syntax to the
    /// given stream.
    #[inline]
    ...
    /// Serialize this node and its descendants in HTML syntax to a new
